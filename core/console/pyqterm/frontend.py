...
from PyQt4.QtCore import QRect, Qt, pyqtSignal, QString
...
class TerminalWidget(QWidget):
    ...
    signal_mouse_down = pyqtSignal(QString, int, int, name="signal_mouse_down")   # line content col, row
            ...
            for item in line:
                if isinstance(item, basestring):
                    ...
                    if str(item).upper().find("ERROR") >= 0:
                        ...
                        error_brush = QBrush(QColor(Qt.red))
    ...
    def mousePressEvent(self, event):
        ...
        if button == Qt.RightButton:
            ...
            if row >=0  and  row< len(self._text):
                self.signal_mouse_down.emit(self._text[row], col, row)
